							...
							<div class="col-md-12">
								...
								<div class="panel panel-white">
									<div class="panel-heading">
										<h4 class="panel-title">Form <span class="text-bold">Lihat</span> Buku</h4>
...
$sql_news="SELECT
id_buku,judul,isi,status,pengarang,stock,picture,pengirim,nama_peminjam,penerbit,date,lihat from tb_buku
INNER JOIN tb_penerbit on tb_penerbit.id_penerbit=tb_buku.id_penerbit
INNER JOIN tb_pengarang on tb_pengarang.id_pengarang=tb_buku.id_pengarang
WHERE status='T' 
ORDER by date"; 
$qry_news=mysql_query($sql_news,$konek)
or die ("gagal menampilkan".mysql_error());
while($hsl_news=mysql_fetch_array($qry_news)) {
$verlijudul=$hsl_news['judul'];
$verlipict=$hsl_news['picture'];
$verlipengarang=$hsl_news['pengarang'];
$verlipenerbit=$hsl_news['penerbit'];
$verlidesc=$hsl_news['isi'];
$verlipengirim=$hsl_news['pengirim'];
$idpenerbit=$hsl_news['id_penerbit'];
$tgl=substr($hsl_news['date'],8,2);  
 $bln=substr($hsl_news['date'],5,2);  
  $thn=substr($hsl_news['date'],0,4); 
$namaurl=preg_replace("/\s/","-",$hsl_news['judul']);
$url_link="detail".$hsl_news['id_buku']."-".$namaurl.".html";
?>
													<tr>
														<?php
														error_reporting(0);
													    ...
													    <td><img src="assets/verlybook/<?php echo $verlipict;?> "width="120" height="120"></td>
														<td><?php echo  $verlijudul;?></td>
														<td><?php echo $verlidesc;?></td>
														<td><?php echo $verlipengarang;?></td>
														<td><?php echo $verlipenerbit;?></td>
														<td><?php echo $bln;?>-<?php echo $tgl;?>-<?php echo $thn;?></td>
														<td><?php echo $hsl_news['stock'];?></td>
														<td>
													<?php if($hsl_news['status']=='T'){ ?>
													<button type="button" class="btn btn-yellow">
																Tersedia 
													</button>
													<?php }else if($hsl_news['status']=='F'){ ?>
													...
													error_reporting(0);
												    session_start();?>
												<a href="ver_pinjamUSER.php?idbukupinjam=<?php echo $hsl_news['id_buku'];?>
												&idnamapem=<?php echo $_SESSION['username'];?>
												&judul=<?php echo $hsl_news['judul'];?>
												&penerbit=<?php echo $hsl_news['penerbit'];?>
												&pengarang=<?php echo $hsl_news['pengarang'];?>
												&stock=<?php echo $hsl_news['stock'];?>
												&isi=<?php echo $hsl_news['isi'];?>
												&picture=<?php echo $hsl_news['picture'];?>
												&id_user=<?php echo $_SESSION['id_login'];?> 
												">
											...
											<?php } ?>
                                               	...
                                               	<td>
											<?php if($hsl_news['status']=='T'){ ?>
											<a href="<?php echo $url_link?>"><button type="button" class="btn btn-light-azure">
										Lihat Buku
											</button></a>
											<?php }else if($hsl_news['status']=='F'){ ?>
											<a href="#"><button type="button" class="btn btn-light-azure">
										Tidak Bisa Di Lihat
													<?php } ?>
										</td>
...
<!-- Mirrored from www.cliptheme.com/demo/rapido/table_export.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 13 Nov 2014 00:38:29 GMT -->
</html>