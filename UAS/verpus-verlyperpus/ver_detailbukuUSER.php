							...
							<div class="col-md-12">
								...
								<div class="panel panel-white">
									<div class="panel-heading">
										<h4 class="panel-title">SPOILER BUKU</h4>
									</div>
...
include "config/liblary.php";
$iddetail=$_GET['iddetail'];
$sql_baca="UPDATE ri_product SET view=view+1
WHERE id_product='$iddetail'";
mysql_query($sql_baca);
$sql_news = "SELECT judul,pengarang,picture,penerbit,isi,pengirim from tb_buku
INNER JOIN tb_pengarang on tb_pengarang.id_pengarang=tb_buku.id_pengarang
INNER JOIN tb_penerbit on tb_penerbit.id_penerbit=tb_buku.id_pengarang 
WHERE id_buku='$iddetail'";
$qry_news = mysql_query($sql_news)
or die ("Gagal query tampil");
$verliqueriarray=mysql_fetch_array($qry_news);
$tgl=substr($verliqueriarray['date'],8,2);   
	$bln=substr($verliqueriarray['date'],5,2);   
	$thn=substr($verliqueriarray['date'],0,4);
$verlijudul=$verliqueriarray['judul'];
$verlipengarang=$verliqueriarray['pengarang'];
$verlipenerbit=$verliqueriarray['penerbit'];
$verlipict=$verliqueriarray['picture'];
$isi=$verliqueriarray['isi'];
$pengirim=$verliqueriarray['pengirim'];
$tgl_ind=substr($data_date,8,2)."-".substr($data_date,5,2)
."-".substr($data_date,0,4);
    ?>
<center><h1>Judul "<?php echo $verlijudul;?>"</h1></center>
							<h4><i><li>Pengirim : <?php echo $pengirim;?></h4></i></li>
						   <h4><i><li>Penerbit : <?php echo $verlipenerbit;?></h4></i></li>
						   	   <h4><i><li>Pengarang : <?php echo $verlipengarang;?></h4></i></li>
						   	   <hr>

<br>
<br>
<center><img src="assets/verlybook/<?php echo $verlipict;?> "width="400" height="400"></center>
<?php echo $verli ?>
<center>
	   <h4><i> <?php echo $isi;?></h4></i></center>
	   <br>
	   <br>
	   <br>
	  <a class="btn btn-primary" href="ver_spoilerbukuUSER.php"><i class="glyphicon glyphicon-log-in"></i></a>
	   									</div>
								</div>
								<!-- end: TIMELINE PANEL -->
							</div>
...
<!-- Mirrored from www.cliptheme.com/demo/rapido/pages_timeline.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 13 Nov 2014 00:39:38 GMT -->
</html>