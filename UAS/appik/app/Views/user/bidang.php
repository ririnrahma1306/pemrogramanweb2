            ...
            <div class="card box" style="width: 15rem;">
                ...
                <?php if ($ik['status'] == 'RESMI') : ?>
                    <div class="btn-group d-flex" style="position: absolute;bottom:45px;width:15rem" role="group" aria-label="Basic mixed styles example">
                        ...
                        <?php if ($ik['disetujui'] == user()->id_bidang && $ik['disusun'] == user()->jabatan) : ?>
                            <a href="<?= base_url('revisi/' . $ik['slug']); ?>" class="btn text-white btn-warning w-100 px-0" type="button">Revisi</a>
                            <form action="<?= base_url('delete/' . $ik['id']); ?>" method="POST" class="btn-group d-flex" role="group" style="width:100%;">
                                <?= csrf_field(); ?>
