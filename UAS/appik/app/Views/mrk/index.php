...
<div class="home-content">
        ...
        <?php foreach ($data as $ik) : ?>
            <div class="card box" style="width: 15rem;">
                ...
                <div class="btn-group d-flex" style="position: absolute;bottom:45px;width:100%;" role="group" aria-label="Basic mixed styles example">
                    ...
                    <form action="<?= base_url('mrk/update'); ?>" method="POST" class="btn-group d-flex" style="width:100%;" role="group">
                        <?= csrf_field(); ?>
                        <input type="hidden" name="setuju" value=" <?= $ik['id']; ?>  ">
                        <button type="submit" class="btn btn-success w-100 px-0" onclick="return confirm('Apakah Anda Yakin?');">Setuju</button>
                    </form>
                </div>
    ...
    <form id="msform" action="<?= base_url('mrk/perbaiki'); ?>" method="POST" enctype="multipart/form-data">
        <?= csrf_field(); ?>
